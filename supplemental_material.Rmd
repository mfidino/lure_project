...
bin_results <- readRDS("det_prob_precip.RDS")
...
tte_results <- readRDS("exponential_results_precip.RDS")

D <- tte_results[,2:4,]

test <- apply(D, c(2,3), HDIofMCMC)
dimnames(test) <- list(1:3, 1:3, species_to_use)
...
pois_results <- readRDS("poisson_results_precip.RDS")
